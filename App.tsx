...
import { generateStreamingResponse, generateConversationalAudioScript, GEMINI_API_KEY } from './services/geminiService';
import { generateAudio, ELEVENLABS_API_KEY } from './services/elevenLabsService';
    ...
    <div className="min-h-screen flex flex-col items-center justify-center text-center p-4">
        <div className="bg-red-900/50 border border-red-500 rounded-xl p-8 max-w-2xl">
            ...
            <p className="mt-4 text-lg text-red-200">
                The application cannot start because the following API key(s) are not configured:
            </p>
            ...
            <p className="text-red-200">Please replace the placeholder values in the corresponding service files to continue.</p>
        </div>
    </div>
);
...
const App: React.FC = () => {
  ...
  const missingApiKeys = useMemo(() => {
    ...
    if (GEMINI_API_KEY === "YOUR_GEMINI_API_KEY_HERE") {
        missing.push("GEMINI_API_KEY in services/geminiService.ts");
    }
    if (ELEVENLABS_API_KEY === "YOUR_ELEVENLABS_API_KEY_HERE") {
        missing.push("ELEVENLABS_API_KEY in services/elevenLabsService.ts");
    }
    return missing;
  }, []);
